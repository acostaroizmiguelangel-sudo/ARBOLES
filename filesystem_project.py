...
class FileSystem:
    ...
    def move_node(self, source_path, destination_path):
        ...
        temp = dest_parent
        while temp:
            if temp == node_to_move:
                print("Error: No se puede mover una carpeta dentro de uno de sus subdirectorios.")
                return False
            temp = temp.parent
    ...
    def list_children(self, path):
        ...
        for child in children_list:
            prefix = "[CARPETA]" if child.tipo == 'carpeta' else "[ARCHIVO]"
            print(f"  {prefix} {child.nombre}")

    def export_preorder(self, path):
        node = self.get_node_by_path(path)
        if not node:
            print("Error: Ruta no válida para recorrido.")
            return
